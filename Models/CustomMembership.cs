    ...
    public sealed class CustomMembershipProvider : MembershipProvider
    {
                 ...
                 out MembershipCreateStatus status)
        {
            ...
            if (u == null)
            {
                ...
                try
                {
                    ...
                    dt = db.Access(sql, username, EncodePassword(password), password, email, passwordQuestion, EncodePassword(passwordAnswer), isApproved, "", createDate, createDate, createDate, pApplicationName, false, createDate, 0, createDate, 0, createDate);
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT IdUser, Username, Email, PasswordQuestion," +
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            {
                db.Start();
                    string sql = "SELECT IdUser, Username, Email, PasswordQuestion," +
                 " Comment, IsApproved, IsLockedOut, CreationDate, LastLoginDate," +
                 " LastActivityDate, LastPasswordChangedDate, LastLockedOutDate" +
                 " FROM Users WHERE Username = @1 AND ApplicationName = @2";

                dt = db.Access(sql, username, pApplicationName);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        sql = "UPDATE Users " +
                                  "SET LastActivityDate = @3 " +
                                  "WHERE Username = @1 AND Applicationname = @2";

                        db.Access(sql, username, pApplicationName, DateTime.Now);
                    }
                }
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            {
                db.Start();
                string sql = "SELECT Password, IsApproved FROM Users " +
                    " WHERE Username = @1 AND IsLockedOut = 'False'";
                dt = db.Access(sql, username);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows[0]["IsApproved"] != DBNull.Value)
                    {
                        isApproved = Convert.ToBoolean(dt.Rows[0]["IsApproved"]);
                    }
                }
                else
                {
                ...
                if (CheckPassword(password, pwd))
                {
                    if (isApproved)
                    {
                        ...
                        sql = "UPDATE Users SET LastLoginDate = @2 " +
                              "WHERE Username = @1";
                        db.Access(sql, username, sqlFormattedDate);
                    }
                }
                else
                {
